...
description: Builds a product metric tree from North Star Metric through L1 drivers, L2 team-owned behaviours, and L3 feature leading indicators, plus structural anti-pattern checks. Use when defining NSM decomposition, team metrics, metric hierarchies, or when the user asks for a metric tree or North Star breakdown.
---
...
To stress-test individual metrics (NSM, L1 drivers, experiments), use [metric-evaluator/SKILL.md](../metric-evaluator/SKILL.md) (unit of value, truth detector, actionable). Optional after the tree: evaluate the NSM and one L1.
...
- Offer to **evaluate the NSM** (and optionally one L1) with the metric-evaluator skill if the user wants deeper critique on definitions, gameability, or ownership.
