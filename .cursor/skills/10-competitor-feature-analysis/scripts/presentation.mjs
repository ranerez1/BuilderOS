...
function cellBadge(value) {
  ...
  if (needMatch) {
    ...
    return `<span class="pill pill-need">needs verify</span>${rest ? `<span class="cell-text">${escapeHtml(rest)}</span>` : ''}`;
  }
  ...
  if (cls) {
    ...
    return `<span class="pill ${cls}">${escapeHtml(label)}</span>${rest ? `<span class="cell-text">${escapeHtml(rest)}</span>` : ''}`;
  }
  return `<span class="cell-text">${escapeHtml(raw)}</span>`;
}
...
function dataQualitySection(dq) {
  ...
  const dqList = [
    ...
    .join('');
  return `<section><div class="kicker">Data quality</div><div class="dq"><ul>${dqList}</ul></div></section>`;
}
...
function renderComparison(data, dataDir, repoRoot) {
  ...
  const body = `${hero}${insights}${scopeSection}${tableSection}${flows}${notesSection}${docsSection}${dataQualitySection(data.dataQuality)}<footer>Generated by /10-competitor-feature-analysis · ${escapeHtml(data.date || '')} · neutral research, no own-product recommendations.</footer>`;
